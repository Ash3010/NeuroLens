...
def get_resnet():
    ...
    if _resnet_model is None:
        _resnet_model = models.resnet50(weights=ResNet50_Weights.DEFAULT)
        _resnet_model.eval()
...
# ── Feature maps ───────────────────────────────
def extract_feature_maps(model, tensor, layer_name):
    activations = {}
...
def make_layer_comparison(model, tensor):
    ...
    for col, layer_name in enumerate(layers):
        ...
        axes[1][col].axis('off')
        axes[1][col].set_facecolor('#12121a')

    fig.tight_layout()
    return fig

# ── Main entry point ───────────────────────────
def run_resnet(img_np, layer_names, num_channels=16):
    model = get_resnet()
    ...
    for layer_name in layer_names:
        ...
        fig = make_feature_map_fig(
            fmap,
            f'Feature Maps — ResNet50 {layer_name} (first {num_channels} channels)',
            num_channels
        )
        feature_maps[layer_name] = fig

    gradcam = make_gradcam(model, tensor, rgb_img)
    layer_comparison = make_layer_comparison(model, tensor)
